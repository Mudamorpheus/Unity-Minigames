...
namespace SmartNS.Editor {
    public class AssetMetaInfo {
        ...
        public string NewNameSpace = string.Empty;
    ...
    public class SmartNS : UnityEditor.AssetModificationProcessor {
        ...
        public static List<AssetMetaInfo> AssetMetaInfos = new();
        ...
        private static Dictionary<string, string> _assetsContent = new();
            ...
            try {
                if (!_currentlyMovingAssets.Contains(sourcePath)) {
                    ...
                    var updateNamespacesWhenMovingScripts =
                        smartNSSettings.FindProperty("m_UpdateNamespacesWhenMovingScripts").boolValue;
                    var enableDebugLogging = smartNSSettings.FindProperty("m_EnableDebugLogging").boolValue;
                    _shouldWriteDebugLogInfo = enableDebugLogging;
                    ...
                    if (updateNamespacesWhenMovingScripts) {
                        ...
                        if (sourcePath.EndsWith(".cs")) {
                            WriteDebug("Moving asset. Source path: " + sourcePath + ". Destination path: " +
                                       destinationPath + ".");
                ...
                Debug.LogError($"Something went really wrong trying to execute SmartNS: {ex.Message}");
                Debug.LogError($"SmartNS Failure Stack Trace: {ex.StackTrace}");
            } finally {
        ...
        public static void UpdateAssetNamespace(string assetPath,
            ...
            _shouldWriteDebugLogInfo = enableDebugLogging;
            WriteDebug($"Acting on new C# file: {assetPath}");
            ...
            var oldNamespaceValue = string.Empty;
            // Generate the namespace.
            var namespaceValue = GetNamespaceValue(assetPath, scriptRootSettingsValue, prefixSettingsValue,
                universalNamespaceSettingsValue);
            ...
            var rawFileContents = System.IO.File.ReadAllText(fullFilePath);
            var rawLines = rawFileContents.Split(new[] {lineEnding}, StringSplitOptions.None);
            ...
            var hasExistingNamespace = false;
            for (int rawLineIndex = 0; rawLineIndex < rawLines.Length; rawLineIndex++) {
                ...
                if (!hasExistingNamespace && line.TrimStart().StartsWith("namespace ")) {
                    ...
                    oldNamespaceValue = line.TrimStart("namespace ".ToCharArray()).TrimEnd('{').Trim();
                    ...
                    while (rawLineIndex < rawLines.Length) {
                        ...
                        if (line.Contains("{")) {
                            ...
                            var curlyBraceOnward = line[indexOfCurlyBrace..];
                            if (indexOfCurlyBrace == 0) {
                                modifiedLines.Add($"namespace {namespaceValue}");
                                modifiedLines.Add(curlyBraceOnward);
                            } else {
                                modifiedLines.Add($"namespace {namespaceValue} {curlyBraceOnward}");
                            }
            ...
            AssetMetaInfos.Add(new AssetMetaInfo {
                AssetPath = assetPath,
                OldNameSpace = oldNamespaceValue,
                NewNameSpace = namespaceValue,
                Content = modifiedLines
            });
        ...
        public static void UpdateUsings() {
            if (AssetMetaInfos?.Count == 0) return;

            // foreach (var metaInfo in AssetMetaInfos) {
            //     var oldNameSpace = metaInfo.OldNameSpace;
            //     var newNameSpace = metaInfo.NewNameSpace;
            //     var assetPath = metaInfo.AssetPath;
            //
            //     foreach (var meta in AssetMetaInfos) {
            //         var content = meta.Content;
            //         var newContent = new List<string>();
            //         foreach (var codeString in content) {
            //             if (codeString.Contains($"using {oldNameSpace};")) {
            //                 var newCodeString = codeString.Replace(oldNameSpace, newNameSpace);
            //                 newContent.Add(newCodeString);
            //             } else {
            //                 newContent.Add(codeString);
            //             }
            //         }
            //         content = newContent;
            //     }
            ...
            if (AssetMetaInfos != null) {
                var newAssetMetaInfos = AssetMetaInfos.ToList();
                var newContent = new List<string>();
                foreach (var metaInfo in AssetMetaInfos) {
                    var oldNameSpace = metaInfo.OldNameSpace;
                    var newNameSpace = metaInfo.NewNameSpace;
                    ...
                    foreach (var newMetaInfo in newAssetMetaInfos) {
                        newContent.Clear();
                        var content = newMetaInfo.Content;
                        foreach (var codeString in content) {
                            if (codeString.Contains($"using {oldNameSpace};")) {
                                var newCodeString = codeString.Replace(oldNameSpace, newNameSpace);
                                newContent.Add(newCodeString);
                        ...
                        newMetaInfo.Content = newContent.ToList();
                    }
                }
                
                
                foreach (var newMetaInfo in newAssetMetaInfos) {
                    var assetPath = newMetaInfo.AssetPath;
                    var content = newMetaInfo.Content;
                    
                    File.WriteAllText(assetPath, string.Join(DetectLineEndings(assetPath), content.ToArray()));
                }
            }
        }
