...
namespace Prime31
{
	...
	public class StoreKitBinding
	{
		...
		public static IEnumerator validateReceipt( bool useSandboxServers, string password = null, Action<StoreKitAppReceipt> completionHandler = null )
		{
			...
			var www = UnityWebRequest.Post( useSandboxServers ? "https://sandbox.itunes.apple.com/verifyReceipt" : "https://buy.itunes.apple.com/verifyReceipt", json );
			yield return www.SendWebRequest();

			if( !www.isNetworkError )
			{
				// why must we dump the JSON to disk to get it to parse properly? Who knows. This isnt production code so it doesnt matter anyway.
				var receiptJsonFile = System.IO.Path.Combine( Application.persistentDataPath, "receipt.json" );
				System.IO.File.WriteAllText( receiptJsonFile, www.downloadHandler.text );
